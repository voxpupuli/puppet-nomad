  ...
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      context 'with server recovery enabled' do
        ...
        it { is_expected.to compile.with_all_deps }
        it { is_expected.to contain_class('nomad::server_recovery') }
        it { is_expected.to contain_file('/tmp/peers.json').with(content: '[
  {
...
', owner: 'root', group: 'root', mode: '0640') }
        it { is_expected.to contain_file('/usr/local/bin/nomad-server-outage-recovery.sh').with(owner: 'root', group: 'root', mode: '0750') }
      end
    end
  end
end
