...
# @param user username
# @param manage_group manage the group that will run nomad
# @param extra_groups additional groups to add the nomad user
# @param purge_config_dir Purge config files no longer generated by Puppet
# @param group groupname
# @param join_wan join nomad cluster over the WAN
# @param bin_dir location of the nomad binary
# @param version Specify version of nomad binary to download.
# @param install_method install via system package, download and extract from a url. Defaults to `url` or disable install.
# @param os operation system to install for
# @param download_url download url to download from
# @param download_url_base download hostname to down from
# @param download_extension archive type to download
# @param package_name Only valid when the install_method == package. Defaults to `nomad`.
# @param package_ensure Only valid when the install_method == package. Defaults to `installed`.
# @param config_dir location of the nomad configuration
# @param extra_options Extra arguments to be passed to the nomad agent
# @param config_hash Use this to populate the JSON config file for nomad.
# @param config_defaults default set of config settings
# @param config_mode Use this to set the JSON config file mode for nomad.
# @param manage_service manage the nomad service
...
# @param service_enable enable the nomad service
# @param service_ensure ensure the state of the nomad service
# @param restart_on_change Determines whether to restart nomad agent on $config_hash changes. This will not affect reloads when service, check or watch configs change.
# @param init_style What style of init system your system uses.
class nomad (
  String[1] $arch,
